...
{	
    ...
    vec3 _L =     normalize(vec3(view*vec4(posFocus, 1.0) - view*TG*vec4(vertex, 1.0)));
    vec3 _norm =  normalize(NormalMatrix*normal);

    vec4 _vertSCO = view * TG * vec4 (vertex, 1.0);
    
    fcolor = Ambient() + Difus(_norm, _L, colorFocus) + Especular(_norm, _L, _vertSCO, colorFocus);
    gl_Position = proj * _vertSCO;
}
