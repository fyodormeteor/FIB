...
void MyGLWidget::initializeGL() {
    ...
    QTimer::connect(&timer, SIGNAL(timeout()), this, SLOT(animar()));
    timer.start(16);
...
void MyGLWidget::modelTransformModelo() 
{
    ...
    TG = glm::scale(TG, glm::vec3(escala_modelo));
...
void MyGLWidget::mouseMoveEvent (QMouseEvent *event) {
    ...
    int current_mouse_x = event->x();
    int current_mouse_y = event->y();

    if (event->buttons() == Qt::LeftButton &&
        !(event->modifiers() & (Qt::ShiftModifier | Qt::AltModifier | Qt::ControlModifier)))
    {
        angle_y -= current_mouse_x - mouse_x;
        angle_x += current_mouse_y - mouse_y;
...
void MyGLWidget::viewTransform(glm::vec3 VRP, float d, float psi, float theta) {
...
void MyGLWidget::updateCamera() {
    ...
    float d = radi_escena * 3.0f;
    viewTransform(VRP, d, angle_y, angle_x);
