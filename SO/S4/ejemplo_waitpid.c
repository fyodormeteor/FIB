  ...
  while (hijos > 0)
    {
      ...
      if (WIFEXITED(res))
      {
        sprintf (buff, "Process %d terminated normally with exit status %d\n", pid, WEXITSTATUS(res));
        write (1, buff, strlen (buff));
      }
      else if (WIFSIGNALED(res))
      {
        sprintf (buff, "Process %d was terminated by signal %d\n", pid, WTERMSIG(res));
        write (1, buff, strlen (buff));
      }
      else
      {
        sprintf (buff, "Process %d was terminated in an unknown way\n", pid);
        write (1, buff, strlen (buff));
      }
